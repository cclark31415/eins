...
41.  (Processed) Foldable phone layout: unfolded inner screens (Pixel Fold, Galaxy Z Fold) are near-square — wide enough to miss the phone layout but far too short for the wide one — and the table was being clipped, cutting off the top bot's name and slicing the current-colour pill in half. Add a layout tier for that shape: side opponents fan sideways instead of downwards so a side seat costs one card of height rather than several, the table closes into a triangle around the pile instead of spreading to the edges of an 8-inch screen, and each side seat's highlight hugs its own hand. Card size is solved from the vertical budget in every tier so the playfield fits the viewport rather than overflowing into the clip.
